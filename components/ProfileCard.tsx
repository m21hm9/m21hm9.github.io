...
export interface ProfileCardProps {
  ...
  onContactClick?: () => void;
  contactHref?: string;
  /** Use a plain white background with no glow/shine effects */
...
function ProfileCardComponent({
  ...
  const getOffsets = useCallback(
    (evt: PointerEvent, el: HTMLElement) => {
  ...
  return (
    <div
      ref={wrapRef}
      className={`pc-card-wrapper ${whiteBackground ? "pc-card-wrapper--white" : ""} ${className}`.trim()}
      style={cardStyle}
    >
      {behindGlowEnabled && !whiteBackground && <div className="pc-behind" />}
          ...
          <div className="pc-inside">
            ...
            <div className="pc-content">
              <div className="pc-details">
                <h3 className="pc-name-two-lines">
                  {(() => {
                    const parts = name.trim().split(/\s+/);
                    if (parts.length <= 1) return name;
                    // Always show "Thom Man Hei" on first line, "Matthew" on second
                    const firstLine =
                      parts[0] === "Matthew"
                        ? parts.slice(1).join(" ")
                        : parts.slice(0, -1).join(" ");
                    const secondLine =
                      parts[0] === "Matthew" ? parts[0] : parts[parts.length - 1]!;
                    return (
                      <>
                        {firstLine}
                        <br />
                        <span className="pc-name-last">{secondLine}</span>
                      </>
                    );
                  })()}
                </h3>
